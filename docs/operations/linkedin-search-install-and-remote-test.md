...
适用版本：Tuolin Marketplace `1.52.2`。
...
解压 `tuolin-marketplace-1.52.2.zip`，假设目录为 `C:\Tuolin\tuolin-marketplace-1.52.2`：

```powershell
codex plugin marketplace add C:\Tuolin\tuolin-marketplace-1.52.2
codex plugin add tuolin-marketplace@tuolin-marketplace
codex plugin add chrome@openai-bundled
codex plugin list
```

安装或升级后新开一个 Codex 会话。若界面版本、`codex plugin list` 与 `1.52.2` 不一致，停止测试并先解决版本冲突。
...
自动测试通过不等于真实业务验收通过。1.52.2 的完整通过条件是：
