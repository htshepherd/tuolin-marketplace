...
from tuolin_marketplace.video_creation_agent import submit_dreamina_jobs


def main() -> int:
    parser = argparse.ArgumentParser(description="Submit Dreamina jobs for a Tuolin video creation run.")
    ...
    result = submit_dreamina_jobs(Path(args.run_dir), execute=args.execute, dreamina_command=args.dreamina_command, now=now)
