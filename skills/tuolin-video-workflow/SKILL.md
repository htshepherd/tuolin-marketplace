...
14. Wait for `确认镜头`.
15. After shots are confirmed, merge the downloaded shot videos into `dreamina_generation/stitched_video.mp4` and generate a CapCut/Jianying manual editing handoff: `voiceover_script.md`, `editing_notes.md`, and `editing_subtitles.md`.
...
`assemble_confirmed_video.py` / `handle_video_creation_reply.py {run_dir} "合并视频"` uses local ffmpeg to concatenate downloaded shot videos in storyboard order. It also writes `voiceover_script.md` for copying into CapCut/Jianying text-to-speech, `editing_notes.md` for the manual editing steps, and `editing_subtitles.md` as a subtitle reference; it does not generate voice, burn subtitles, add BGM, or publish.
