...
- Prompt 只在分镜与 `storyboard.srt` 共同确认后生成。Prompt 是给即梦使用，不是字幕、旁白或发布文案。
- `storyboard.srt` 是未来旁白唯一逐字稿；Prompt 不得改写、补充或替代 SRT，也不得把运镜描述当成字幕。
- 产品可见镜头必须使用真实产品图片或视频作为参考。
- 每个进入即梦任务的素材必须有稳定编号和明确用途；不得只写“参考素材”或“参考 @图片1”。
- `prompts.md/json` 与 `dreamina_jobs.md/json` 必须使用同一套有序素材编号映射，并保留每张参考图的连续性角色。
...
- Prompt 转换不能改变已确认镜头的目的、动作、顺序、参考图或 SRT 表达。provider 无法执行时必须回到分镜修改并重新确认。
