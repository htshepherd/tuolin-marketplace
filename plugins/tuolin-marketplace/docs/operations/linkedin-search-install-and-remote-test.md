...
或解压 `tuolin-marketplace-2.0.2.zip` 后从本地目录安装：

```powershell
codex plugin marketplace add C:\Tuolin\tuolin-marketplace-2.0.2
codex plugin add tuolin-marketplace@tuolin-marketplace
codex plugin add chrome@openai-bundled
codex plugin list
```

安装或升级后新开 Codex 会话。若界面版本与 `codex plugin list` 不是 `2.0.2`，停止业务测试并先解决版本冲突。
